    ...
    var MdSliderPanel = function() {
        ...
        this.initTab = function() {
            self.tabs = $("#md-tabs").tabs({
                tabTemplate: '<li class="tab-item first clearfix"><a class="tab-link" href="#{href}"><span class="thumb-disable"></span><span class="tab-text">#{label}</span></a> <span class="ui-icon ui-icon-close">Remove Tab</span></li>',
            ...
            $(".md-tabs-head span.ui-icon-close").live("click", function() {
                ...
                if (settings.slide_id == -1) {
                    ...
                    $.post( Drupal.settings.slider.delete_slide, {sid: settings.slide_id}, function(data_response) {
                        if (data_response == 'OK') {
                ...
                buttons: {
                    Save: function() {
                        ...
                        if($tab) {
                            ...
                            //add slide backgound video
                            if(settings.mp4_link || settings.webm_link){
                                var $video = $('<video playsinline autoplay muted loop style="width:100%; height: auto;">\
                                                <source src="' + settings.webm_link + '" type="video/webm">\
                                                <source src="' + settings.mp4_link + '" type="video/mp4">\
                                              </video>');
                                $('.md-slide-image img, .md-slide-image video', self.activePanel).remove();
                                $('.md-slide-image', self.activePanel).append($video);
                                
                            } else {
                                // Add slide background image
                                $.post( Drupal.settings.slider.get_image, {fid: settings.background_image, slider_id: slid}, function(response) {
                                    if (response) {
            ...
            $("#background-video").dialog({
                ...
                buttons: {
                    OK: function () {
                        var $tab = $("#slide-setting-dlg").data("tab");
                        if($tab) {
                            var settings = {
                                    mp4_link: $("#mp4_link").val(),
                                    webm_link: $("#webm_link").val(),
                                },
                                old_settings = $.stringToObject($("input.panelsettings", $tab).val());
                ...
                open: function() {
                    ...
                    if($tab) {
                        var settings = $("input.panelsettings", $tab).val();
                        (settings != "") && (settings = $.stringToObject(settings));
                        $("#mp4_link").val(settings.mp4_link);
                        $("#webm_link").val(settings.webm_link);
                    }
                },
                close: function() {
                    $(this).empty();
            ...
            $('.show-video-box').live('click', function() {
                var contentVideoBox = '<div class="dlg-inner">\
                                            <form id="form-slider-panelsetting">\
                                                <fieldset class="ui-helper-reset">\
                                                    <div class="form-item">\
                                                        <label for="videoid">Mp4 video link</label>\
                                                        <input type="text" name="mp4_link" id="mp4_link" class="form-text" autocomplete="false" value="" />\
                                                    </div>\
                                                    <div class="form-item">\
                                                        <label for="videoname">Webm video link (fix for some browsers)</label>\
                                                        <input type="text" name="webm_link" id="webm_link" class="form-text" autocomplete="false" value="" />\
                                                    </div>\
                                                </fieldset>\
                                            </form>\
                                        </div>';
                $("#background-video").append(contentVideoBox).dialog("open");
            });
        ...
        this.setBackgroundVideo = function(){
        }
        ...
        this.setSlideSettingValue = function(setting) {
            ...
            if (setting && setting.custom_thumbnail != -1) {
                ...
                $.post( Drupal.settings.slider.get_image, {fid: setting.custom_thumbnail, slider_id: slid}, function(response) {
            ...
            if (setting && setting.background_image != -1) {
                ...
                $.post( Drupal.settings.slider.get_image, {fid: setting.background_image, slider_id: slid}, function(response) {
