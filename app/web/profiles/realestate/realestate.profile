 ...
 */
function realestate_form_install_configure_form_alter(&$form, $form_state) {
  // Pre-populate the site name with the server name.
  $form['site_information']['site_name']['#default_value'] = $_SERVER['SERVER_NAME'];
  $form['#submit'][] = 'realestate_form_submit';
}


function realestate_form_submit($form, &$form_state) {
  // Import database realestate file.
  $realestate_file = dirname(__FILE__) . '/realestate.sql';
  $success = import_realestate($realestate_file);
...
/**
 * Imports a database realestate file.
 *
 * @see demo_reset().
 */
function import_realestate($filename) {

  global $databases;
  $db_prefix = $databases["default"]["default"]["prefix"];
  
  // Open realestate file.
  if (!file_exists($filename) || !($fp = fopen($filename, 'r'))) {
    drupal_set_message(t('Unable to open realestate file %filename.', array('%filename' => $filename)), 'error');
    return FALSE;
  }
  ...
  // Load data from realestate file.
  $success = TRUE;
  $query = '';
  $new_line = TRUE;
